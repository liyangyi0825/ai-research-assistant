...
const BILLING_MIGRATION_FILES_BY_RANGE = {
  ...
  "019": ["202609100019_monthly_semester_catalog.sql"],
} as const;
...
export function buildUpgradePlan(input: PlanInput): readonly PlannedCommand[] {
  ...
  return [
    ...
    planned("copy-migration-017", INTERNAL_EXECUTABLE, ["copy-migrations", "017", "upgrade-workspace"], { cwd: input.runDirectory }),
    linkedRefCheck("verify-upgrade-workspace-ref-before-017", workspace, input.restoreRef, true),
    ...
    planned("copy-migration-019", INTERNAL_EXECUTABLE, ["copy-migrations", "019", "upgrade-workspace"], { cwd: input.runDirectory }),
    linkedRefCheck("verify-upgrade-workspace-ref-before-019", workspace, input.restoreRef, true),
    planned("push-migration-019", "supabase", ["db", "push", "--linked"], {
      cwd: workspace, env: linkEnv, targetRef: input.restoreRef,
    }),
...
export function selectMigrationFiles(
  ...
  switch (range) {
    ...
    case "017":
      ...
      break;
    case "019":
      expectedNames = BILLING_MIGRATION_FILES_BY_RANGE["019"];
      break;
    case "018":
