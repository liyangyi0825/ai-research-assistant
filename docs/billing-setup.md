...
`WECHAT_PAY_MCH_PRIVATE_KEY` 是商户私钥，`WECHAT_PAY_MCH_SERIAL_NO` 是商户证书序列号；这是本项目实际读取的名称。`WECHAT_PAY_PLATFORM_CERT` 用于平台证书/公钥材料。`ALIPAY_APP_PRIVATE_KEY` 是本项目实际读取的应用私钥变量。所有值只应通过受控的服务端密钥管理注入。
