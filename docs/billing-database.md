...
007 会向前重建五个管理写 RPC，供已经应用旧版 005 的数据库获得完整载荷幂等
校验及售后订单锁定；升级时必须按 005、006、007 的顺序执行。
008 必须随后执行，用精确的 `(UUID, BIGINT, TEXT, TEXT, UUID, TEXT)` 签名撤销旧版
006 曾授予 legacy 调额 RPC 的全部外部执行权限；完整顺序为 005→006→007→008。
