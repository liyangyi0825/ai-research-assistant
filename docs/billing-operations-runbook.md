...
以下顺序是生产变更的审批清单，尚未执行。旧应用按数据库读取活跃商品；若生产已为 `CREDIT_PACK_100` 设置 `BILLING_REAL_PAYMENT_PUBLIC_ENABLED=true`，迁移 `019` 激活月/学期商品后，普通用户即可创建订阅购买，早于新代码部署和受控验收。不能将迁移获批视为公开销售获批。

1. **迁移前停售单独获批并验证后，才可备份和执行 `019`。** 先只读记录生产实例、当前 release/commit、公开支付配置、订单/支付/退款/发票进行中清单和回调积压；确认无进行中的普通用户支付、退款或发票操作，或者另行批准有起止时间、影响范围、负责人和恢复条件的维护窗口。窗口必须明确普通用户已有支付查询及售后提交会暂时不可用，并安排对账与人工受理；不得将只隐藏购买按钮当成停售。获批后在所有生产服务实例上生效并验证 `BILLING_REAL_PAYMENT_PUBLIC_ENABLED=false`，保持现有 `BILLING_FEATURE_ENABLED=true`、微信模式及验签配置，使管理员和明确列出的测试账号可用于后续受控验收。备份可恢复性、迁移文件校验和与迁移执行仍须独立批准。
2. **确认停售影响及已验签回调连续性。** 该公开支付开关会拒绝普通用户新订单/创建支付、支付 GET/查询、退款申请、发票申请，包含既有 credits 包流程；不能宣称保留全部售后操作。已验签支付回调不使用普通用户公开支付门禁，必须继续正常结算并保留审计。先用对应版本的离线/隔离验证证据核对这些行为，再在批准窗口只读监测已验签回调、进行中账务和错误情况；若不能确认回调连续性或维护影响未获批准，停止迁移。不得通过额外真实支付/退款来验证，除非另获批准。
3. **迁移后只读核验。** 检查三个商品精确定义、13 项额度关系、约束/触发器/锁、RPC 与权限、迁移历史及校验和。普通用户公开支付继续关闭；不在生产运行会创建测试订单的数据库演练。
4. **候选部署与切换分别获批。** 新代码只能先上传到独立目录、构建并启动 canary，保留当前 release 服务；校验健康、商品目录和公开支付仍关闭后，才能申请生产切换。切换批准必须包含确切旧 release、生产进程/上游、自动回滚动作及健康失败阈值；故障恢复旧应用时公开支付仍须关闭，避免已激活的订阅被旧代码重新公开销售。staging 的目录、进程、端口和域名不得改动，不使用 `pm2 reload all`。
5. **受控真实支付验收另行获批。** 只允许指定管理员/测试账号验证月套餐与学期套餐的金额、期限、权益、额度和幂等性；普通用户继续处于已批准的维护限制。真实订单、扫码、收款、退款各遵守对应批准，退款不能由收款批准推定。
6. **恢复公开销售另行批准。** 新代码、canary、切换和受控验证完成并完成账务对账后，才可申请普通用户公开销售及保持两个订阅商品活跃的批准。批准范围明确包含恢复支付查询、退款/发票申请和 credits 包购买；随后才可恢复 `BILLING_REAL_PAYMENT_PUBLIC_ENABLED=true` 并结束维护。若只需恢复 credits 包而订阅尚未获批，现有全局开关不能隔离 SKU，应保持关闭并另行设计、验证和审批恢复方案，不直接改商品行绕过迁移。
