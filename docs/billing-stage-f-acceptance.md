...
F1 全程保持 `BILLING_FEATURE_ENABLED=false`。若已在获批的独立预发布数据库应用迁移 `019`，目录可保持其三个批准商品为 active；收费开关仍必须在服务端阻止购买。不得为了 F1 手工改写商品状态或把预发布迁移解释为生产迁移。
...
1. 在明确批准的独立预发布数据库备份并按顺序应用迁移 `001`–`019`，再只读核对迁移历史、目录、约束和 RPC；
2. 核对仅 `PRO_MONTHLY`、`PRO_SEMESTER` 和 `CREDIT_PACK_100` 为批准活跃商品，禁止 `PRO_YEARLY`、`FREE` 商品及其他 SKU；
3. 仅在预发布环境临时设置 `BILLING_FEATURE_ENABLED=true`；
4. 仅允许管理员和 `BILLING_TEST_USER_IDS` 执行 Mock 购买；
5. 验收结束立即恢复 `BILLING_FEATURE_ENABLED=false`；商品是否变更必须由新的迁移和独立审批决定，不得直接改生产行。

F2 全程保持 `PAYMENT_MODE=mock` 且所有微信、支付宝真实凭据为空。下面的购买、确认与结算均使用受控 Mock 链路，不创建微信 Native 支付、不生成真实支付二维码，也不发起真实金额交易。真实微信 Native 购买验证只能在 F2 之外，通过独立的商户测试或生产审批门禁执行。

批准后需要验证：

### Pro Monthly

- [ ] 服务端价格为 1990 个 CNY 最小货币单位（19.90 元），有效期 30 天，权益版本为 `pro-v1`。
- [ ] 使用与学期套餐相同的 13 个功能键，订单和结算只采用服务端不可变快照。
- [ ] 通过受控 Mock 一次性支付创建订阅，且 `auto_renew=false`。

### Pro Semester

- [ ] 服务端价格为 7900 个 CNY 最小货币单位（79.00 元），有效期 150 天，权益版本为 `pro-semester-v1`。
- [ ] 13 项功能键与 `PRO_MONTHLY`（`pro-v1`）完全相同，每项周期额度严格为对应月套餐额度的 5 倍，并在一个完整 150 天周期内共享，不按月重置。
- [ ] 通过受控 Mock 一次性支付创建订阅，且 `auto_renew=false`。
- [ ] 客户端伪造价格、币种、期限和额度均无效。
- [ ] 支付成功只创建一次订阅、权益和额度周期。
...
- [ ] 服务端价格为 990 个 CNY 最小货币单位（9.90 元），权益版本为 `credit-v1`，成功后只增加一次 100 credits。
- [ ] 重复通知不重复增加额度。
- [ ] 额度不足时任务不会执行。
...
- [ ] 尚未到期的 `ACTIVE` 订阅或另一笔尚未过期的 `PENDING` 订阅订单会阻止新的月套餐或学期套餐订单，并稳定返回 `409 ACTIVE_SUBSCRIPTION_EXISTS`。
- [ ] 上述订阅门禁期间 `CREDIT_PACK_100` 仍可购买。
- [ ] 完全未使用的月套餐与学期套餐只允许全额原路退款；任一相关额度已经使用时不得自动退款，必须进入人工审核。
...
数据库验收只在获得具体测试项目授权后执行。必须核对 Project Ref、001–019 迁移一致、`db push --dry-run` 无待执行项、完整 `verify.sql` 最终回滚、无合成数据残留、关键触发器启用且内部函数权限关闭。不得把测试项目授权解释为生产数据库授权，也不得声称生产已迁移或部署。
...
- [ ] 生产迁移 `019`、应用部署、真实凭据注入、F2 之外的商户/生产微信 Native 受控真实金额支付验证、受控退款、公开购买与最终保持三个批准商品活跃均已分别审批；任一批准不得替代其他批准。
