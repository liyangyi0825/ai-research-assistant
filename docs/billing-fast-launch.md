...
月/学期套餐迁移必须先完成获批的**迁移前停售**，操作顺序与影响见 [迁移 019 操作门禁](billing-operations-runbook.md#迁移-019迁移前停售与恢复门禁)。如果生产已为 `CREDIT_PACK_100` 开启公开真实支付，旧应用会读取迁移新增的活跃商品；先执行 `019` 会立即放开订阅销售，不能等待新代码部署后再关入口。`BILLING_REAL_PAYMENT_PUBLIC_ENABLED=false` 同时阻止普通用户支付 GET/查询、退款申请和发票申请，不能描述为无感停售；必须确认无相关进行中操作，或另行批准明确承担这些影响的维护窗口。已验签支付回调继续结算。新代码、canary 和受控验证完成后，仍须另行批准公开销售才能恢复该开关；staging 不在变更范围内。
