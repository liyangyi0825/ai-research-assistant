...
SET search_path = pg_catalog, public AS $$
DECLARE v_audit UUID; v_subscription UUID; v_existing JSONB; v_start TIMESTAMPTZ := now(); v_end TIMESTAMPTZ;
BEGIN
  ...
  SELECT jsonb_build_object('status','ALREADY_APPLIED','audit_id',id,
    'resource_id',after_value->>'subscription_id') INTO v_existing
  ...
  INSERT INTO public.billing_admin_audit_logs(
    ...
    NULL,jsonb_build_object('subscription_id',v_subscription,'plan_id',p_plan_id,
      'ends_at',v_end,'idempotency_key',p_idempotency_key)) RETURNING id INTO v_audit;
...
SET search_path = pg_catalog, public AS $$
DECLARE v_request public.billing_refund_requests%ROWTYPE; v_audit UUID; v_existing JSONB;
BEGIN
  PERFORM public.billing_require_write_admin(p_admin_user_id);
  ...
  SELECT jsonb_build_object('status','ALREADY_APPLIED','audit_id',id,'resource_id',target_id)
    INTO v_existing FROM public.billing_admin_audit_logs
  ...
  VALUES(p_admin_user_id,v_request.user_id,'REVIEW_REFUND','REFUND_REQUEST',p_request_id::TEXT,btrim(p_reason),
    jsonb_build_object('status',v_request.status),jsonb_build_object('status',p_decision,'idempotency_key',p_idempotency_key))
...
SET search_path = pg_catalog, public AS $$
DECLARE v_request public.billing_invoice_requests%ROWTYPE; v_audit UUID; v_existing JSONB;
BEGIN
  PERFORM public.billing_require_write_admin(p_admin_user_id);
...
DECLARE v_id UUID; v_audit UUID; v_before JSONB; v_existing JSONB;
BEGIN
  PERFORM public.billing_require_write_admin(p_admin_user_id);
  ...
  VALUES(p_admin_user_id,'UPSERT_PLAN','PLAN',v_id::TEXT,btrim(p_reason),v_before,
    jsonb_build_object('plan_id',v_id,'idempotency_key',p_idempotency_key)) RETURNING id INTO v_audit;
...
BEGIN
  ...
  VALUES(p_admin_user_id,'UPSERT_PRODUCT','PRODUCT',v_id::TEXT,btrim(p_reason),v_before,
    jsonb_build_object('product_id',v_id,'idempotency_key',p_idempotency_key)) RETURNING id INTO v_audit;
  RETURN jsonb_build_object('status','APPLIED','audit_id',v_audit,'resource_id',v_id);
END; $$;
