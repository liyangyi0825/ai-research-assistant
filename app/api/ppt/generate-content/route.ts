...
✅ 每个 paragraphs 条目 = 一个短要点，格式：以 • 开头，尽量不超过 25 字（上限，不是硬指标——不要为凑 5 条或卡 25 字而丢关键因果逻辑，核心数据和推理链必须保留）。

1. 必须从论文中提取真实数据：参数、数值、对比结果，数字要精确
2. content 页用 3-5 个精炼要点（paragraphs 数组 3-5 条），每条格式：• + 核心信息，尽量控制在 25 字以内，核心信息不能丢
...
- 叙述性背景/方法/步骤 → standard 版式

【不同类型页面的具体要求（每条以 • 开头，不超过25字，必须含具体数据）】

研究背景页（content 类型，standard版式，3-4条）：
• 要点1：领域现状 + 关键引用数据（数字必须保留）
• 要点2：现有方法/技术的核心缺陷（一句话说清楚）
• 要点3：本文解决的问题（一句话点题）
• 要点4（可选）：本文方法/思路的核心一句话

研究方法页（content 类型，standard版式，3-5条）：
• 每条 = 一个步骤 + 关键参数数值，格式："步骤名：参数值"
• 禁止写"首先……然后……最后……"，直接写步骤+数值
• 示例："浆料配制：固含量100 mg/mL，NMP为溶剂"

实验结果页（content / figure / stats / table 类型）：
• 单一最重要数据 → hero版式，paragraphs[0]直接写该数据结论（≤25字，含数值）
• 多组对比数据 → comparison 或 split版式，分列对比
• 有表格数据 → table类型直接呈现，禁止放进content段落里文字描述

结论页（content 类型，split版式优先，3-4条）：
• 每条 = 一个创新点，格式："• 提出/实现/验证了 + 效果数据"
• 示例："• 提出高固含量浆料工艺，成本降低40%"
• 每条必须有可量化结果，宁少勿多
...
4. content（内容页，3-5个短要点，每条以 • 开头、不超过25字，禁止整段照搬原文）
{ "type": "content", "layout": "standard", "title": "硅基负极材料的挑战", "paragraphs": ["• 硅理论比容量4200 mAh/g，是石墨的11倍", "• 充放电体积膨胀高达300%，导致材料粉化", "• SEI膜反复破裂，容量快速衰减"], "notes": "重点讲硅的容量优势和体积膨胀这个核心矛盾" }

⚠️ 对比好坏示例：
❌ 错误（照搬原文长段）："本研究在充分分析现有硅基负极材料所面临的体积膨胀问题基础上，提出了一种基于高固含量浆料工艺的纳米硅碳复合制备方法，在保证材料性能的同时大幅降低制备成本"
✅ 正确（提炼3条短要点）：["• 高固含量浆料（100 mg/mL）制备纳米硅/碳", "• 成本显著低于传统纳米化/碳包覆工艺", "• 性能保持率与低成本兼顾"]
...
▸ "standard"（其余叙述页用）
  - 用于：研究背景、文献综述、研究动机、实验方法/步骤等纯叙述性内容
  - 这类页面没有明确"发现/结论"，只是在交代背景或过程
