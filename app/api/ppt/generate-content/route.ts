...
【内容质量要求——最高优先级，违反视为错误输出】
⛔ 禁止整段照搬论文文字。PPT 不是论文翻译，不要把原文段落直接复制进去。
⛔ 禁止只写干巴巴结论，不带数据和因果。"• SEI膜反复破裂"是废话；"• SEI膜反复破裂，持续消耗电解液，循环50次后容量保持率降至60%"才有信息量。
⛔ 禁止每页只有标题没有正文。所有 content 页的 paragraphs 必须有 4-6 条实质内容。
⛔ 禁止生成空白占位符（如"请插入图片"、"待补充"、"[图片]"等）。没有对应图片时，改用文字流程图（card 版式 + flow:true）呈现步骤或流程。
...
研究背景页（content 类型，standard版式，必须严格按因果逻辑写 4-5 条）：
⚠️ 必须体现三层逻辑链，缺一不可：
  ① 痛点/现有问题：当前领域/技术存在什么缺陷？（从论文引言提取，保留数据和数量级）
  ② 研究动机：这个缺陷为什么重要/有害？会导致什么后果？
  ③ 本文方案：本文提出了什么具体解决思路？（一句话点题，不要空泛）
示例结构：
• 要点1：领域规模/意义 + 关键引用数据（交代研究背景价值）
• 要点2：现有方案的核心缺陷 + 缺陷带来的具体后果（有数字更好）
• 要点3：痛点的根本原因分析（为什么现有方案解决不了）
• 要点4：本文提出的解决思路（方法核心一句话）
• 要点5（可选）：本文的预期贡献或验证目标

研究方法页（content 类型，建议 card 版式分步骤，4-6条 或 2-3张卡片）：
• 每条/每张卡片 = 一个步骤 + 关键参数数值 + 为什么这样设置（如有）
• 禁止写"首先……然后……最后……"，直接写步骤+数值
• 没有对应实验图片时，用 card 版式 + flow:true 生成文字流程图，不要留图片占位符
• 示例："浆料配制：固含量100 mg/mL（NMP为溶剂），高固含量减少烘干时间约40%"

实验流程/方法流程（优先用 card+flow:true 版式，而非 figure 类型）：
• 如果论文描述了实验步骤/制备流程/研究流程，用 card+flow:true 呈现（不要留"请插入图片"占位符）
• 每张卡片对应一个步骤，heading 用 ①②③ 序号，points 写 2-3 条关键参数或操作细节
• 只有论文中真实存在的图表才生成 figure 类型页面
...
• 每页必须有实质性数据内容，不允许只有标题或空洞描述

结论页（content 类型，split版式优先，必须 4-5 条有实质内容的要点）：
• 每条 = 一个创新点/主要发现 + 效果数据，格式："• 提出/实现/验证了 X，结果/效果 Y"
• 示例："• 提出高固含量浆料工艺，制备成本较传统工艺降低40%，性能保持率≥90%"
• 每条必须有可量化结果或具体发现，禁止写"本文研究了……""工作具有重要意义"等空话
• 如果论文结论部分内容丰富，可拆成"主要结论"+"展望"两页，各 3-4 条

其他所有 content 页（如文献综述、理论分析、讨论页）：
• paragraphs 至少 4 条，每条含具体信息，不得只写标题或笼统描述
• 从论文对应章节提取原文数据和表述，改写为 PPT 要点格式

【可用的幻灯片类型（9种）】

1. cover
⚠️ author 字段必须从论文内容中提取作者姓名和导师姓名：
- 作者姓名：通常出现在论文标题下方、作者列表第一位
- 导师姓名：通常出现在致谢、作者单位或通讯作者标注中（"*"或"corresponding author"）
- 职称：从论文中提取（教授/副教授/研究员等），提取不到则省略
- 提取不到任何信息时，留占位符："汇报人：XXX\\n指导教师：XXX 教授"
{ "type": "cover", "title": "论文完整标题（从论文提取，不要改写）", "subtitle": "专业/课题组名称（从论文提取，提取不到则留空字符串）", "author": "汇报人：[从论文提取的第一作者姓名]\\n指导教师：[从论文提取的导师姓名及职称]", "date": "${today}" }
