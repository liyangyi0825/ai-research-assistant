...
【figure 页——必须生成，且不计入 15-20 页限制】
- 论文正文中出现的每一个 Figure/Table/图/表，都必须生成 1 个对应的 type="figure" 页面，⛔ 不得省略、不得只挑部分生成
- figure 页数量不受 15-20 页限制约束，可以在此基础上按图表实际数量额外增加
- figure_desc 必须写清图表展示的具体内容和数值（不少于 60 字）
- analysis 必须写清该图表反映出的规律/结论及其意义（不少于 60 字）
