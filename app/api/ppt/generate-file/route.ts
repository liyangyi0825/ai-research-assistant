...
function _renderContentStandard(prs: PptxGenJS, s: ContentSlide) {
  ...
  const contentH = H - 1.2 - 0.15;  // available height for paragraphs
  const paraGap = 0.22;
  const paraH = (contentH - paraGap * Math.max(n - 1, 0)) / Math.max(n, 1);
  const fSize = 14.5;

  paras.forEach((para, i) => {
    const py = 1.2 + i * (paraH + paraGap);
    slide.addText(
      buildRuns(para, { fontSize: fSize, color: C.TEXT, fontFace: "微软雅黑" }),
      opt({ x: 0.35, y: py, w: 9.3, h: paraH, align: "justify", valign: "top", wrap: true, lineSpacingMultiple: 1.8 })
    );
  });
}
...
function _renderContentSplit(prs: PptxGenJS, s: ContentSlide) {
  ...
  paras.forEach((para, i) => {
    const py = 0.2 + i * (paraH + paraGap);
    slide.addText(
      buildRuns(para, { fontSize: fSize, color: C.TEXT, fontFace: "微软雅黑" }),
      opt({ x: 4.1, y: py, w: 5.7, h: paraH, align: "justify", valign: "top", wrap: true, lineSpacingMultiple: 1.8 })
    );
  });
}
