...
【第一步：判断数据结构类型】
在做任何处理前，先判断数据属于哪种结构：
1. 标准表格：每列有唯一列名，每行是一条记录
2. 横向拼接：多组相同结构的数据横向排列，列名在表头中重复出现（如出现多个"一线"、"时间"）
3. 宽表格：一个指标按时间展开成多列
4. 混合结构：以上组合

判断依据：
- 若发现有列名重复出现（包括 pandas 自动加的 .1/.2 后缀，或"时间_重复"等），判定为横向拼接
- 若表头第一行出现日期或 Excel 日期序号后紧跟相同的子列名，判定为横向拼接

【第二步：横向拼接数据的处理规则】
若判定为横向拼接结构，必须：
- 识别每组数据的标识符（通常是每组开头的日期、编号或序号列）
- 重命名时给每组子列加上"组标识_"前缀，格式：组标识_子列名(单位)
  示例：1月1日组的"一线"→ 1月1日_一线(MW)；2月10日组的"一线"→ 2月10日_一线(MW)
- 删除重复的时间/索引列，只保留一个，命名为"时间"
- 组标识列本身（如 Excel 日期序号列）也要给出明确 renamed 名称（如"日期标记_1月1日"），不能遗漏

关于 Excel 日期序号：序号从 1900-01-01=1 开始计数。
换算示例：46023→1月1日，46063→2月10日，46112→3月31日。
精确换算，用"M月D日"格式，不要保留数字序号。

【第三步：通用保护规则】
无论何种结构，重命名后必须保证：
- 所有 renamed 列名全局唯一，绝不允许两列同名
- 若重命名后仍有重复，在列名后加 _2、_3 区分
- 保留原始数据的业务含义，不要用"列1"、"数据1"等无意义编号替换
- issues 中说明检测到的结构类型（如"检测到 3 组横向拼接数据，已自动分组命名"）
