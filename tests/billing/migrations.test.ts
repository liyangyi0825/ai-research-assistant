...
test("catalog reads are enabled-only and authenticated users can only read their own data", () => {
  ...
  assert.match(sql, /billing_plans[\s\S]*using \(is_active = true\)/);
  assert.match(sql, /billing_products[\s\S]*using \(is_active = true\)/);
  ...
  for (const table of [
    ...
    assert.match(
      sql,
      new RegExp(
        `create policy ${table}_select_own on public\\.${table}[\\s\\S]*?auth\\.uid\\(\\) = user_id`,
      ),
    );
  }
...
test("every billing RPC is SECURITY DEFINER with a fixed search path", () => {
  ...
  for (const rpc of rpcNames) {
    assert.match(
      sql,
      new RegExp(
        `create or replace function public\\.${rpc}\\([\\s\\S]*?security definer set search_path = pg_catalog, public`,
      ),
    );
  }
});
